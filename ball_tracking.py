...
ap.add_argument("-b", "--buffer", type=int, default=64,
	help="max buffer size/tail length")
...
ap.add_argument("-V", "--verbose", action='store_true', help="Output coordinates of target") 
	...
	if len(cnts) > 0:
		...
		if radius > args["radius"]:
			...
			cv2.circle(frame, (int(x), int(y)), int(radius),
				(0, 255, 255), 2)
			cv2.circle(frame, center, int(args["radius"]), (0, 0, 255), -1)
